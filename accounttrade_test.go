...
func TestAccountTradeGet(t *testing.T) {
	...
	_, err := client.Accounts.Trades.Get(
		context.TODO(),
		"x",
		"x",
	)
...
func TestAccountTradeListWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Accounts.Trades.List(
		context.TODO(),
		"x",
